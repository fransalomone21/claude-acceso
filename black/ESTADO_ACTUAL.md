...
| **Lista de personajes por unidad: registros de paso `0xB0`**, con el nombre en `+0x00` y **el índice de tipo en `+0x88`**. Cuelgan de `unidad+0x18` (enemigos, cantidad en `+0x20`) y `unidad+0x1C` (compañeros, cantidad en `+0x24`); las unidades son de paso `0x28` y cuelgan de `stage+0x04`, cantidad en `stage+0x08` | **2026-08-21.** `probable` — decompilado de `FUN_001E2D38`, la única referencia de `'Enemy%d_%s'`. Nadie escribió nada todavía. Volcado en `volcados/7b/fun-001e2d38.c` |
