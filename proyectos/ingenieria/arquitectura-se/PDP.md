...
| P6 criterio de entrada al registro | b | **`no aplica` — todavía** | Diseñada y no construida. **La resta:** los cinco criterios de Rechtin (p. 33-34) compran que una preferencia no entre como lección, y cuestan un paso más en cada `aprender.py agregar`. Se difiere porque el registro está sano —210 lecciones, **todas** con triage— así que el problema que P6 ataca no está midiendo daño hoy, y la fase 6 ya gastó su margen en las piezas que sí tenían daño medido |
| P7 separación System 2 / System 3 | b | **`no aplica` — todavía** | Diseñada y no construida. **La resta:** la separación compra una respuesta simple a *¿esto es del método o del proyecto?*, y cuesta reescribir la cascada de seis niveles, que es lo que el criterio C3 del trade study penaliza explícitamente. Se difiere a la fase 7: la pregunta que resuelve se contesta hoy en la práctica sin ambigüedad (`perfil-global/` o el proyecto), así que es la pieza con menos daño medido de las diez |
