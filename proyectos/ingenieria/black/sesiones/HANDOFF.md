...
## 9. TEXTURAS — línea nueva, abierta el 2026-09-02. La fuente es `docs/09-remaster-visual.md`

**Esta sección NO se extiende.** Todo el detalle vive en
`black/docs/09-remaster-visual.md`, que está enlazado desde el contrato del proyecto. Acá va
sólo lo que una sesión nueva necesita para no repetir trabajo ni pisar nada.

**Lo cerrado, confirmado por efecto:** había **8225 texturas `.dds` DXT5 (1305 MB, mtime
23/10/2022)** en el disco desde junio, en `C:\Program Files\PCSX2\PCSX2\textures\SLUS-21376\`,
y **nunca cargaron** porque PCSX2 lee del *data dir*, que es `C:\Users\frans\Documents\PCSX2\`
(medido por mtime del `.ini`, no supuesto) y ahí `textures\` estaba vacía. Copiadas al lugar
correcto, `emulog.txt` emite `Disabling autogenerated mipmaps on one or more compressed
replacement textures`. Fran validó jugando: *"se ve bien"*.

**Estado de la máquina que esto deja:**
- `C:\Users\frans\Documents\PCSX2\textures\SLUS-21376\replacements\` = 8225 archivos, 1305,5 MB.
  **Para desactivar el pack alcanza con renombrar esa carpeta** — es también la rama B del A/B.
- El original sigue intacto en `C:\Program Files\PCSX2\PCSX2\textures\`. Son dos copias: 2,6 GB.
- `LoadTextureReplacements = true` (ya estaba), `PrecacheTextureReplacements = false`,
  `DumpReplaceableTextures = false`.
- Captura de la primera corrida: `pruebas/texturas-hd-primera-corrida.png`.

**DO NOT REPEAT:**
- **No leer los números de OSD de esa corrida como costo del pack.** `GS 22,3 %`, `29,93 fps`:
  fue **sin turbo**, y bajo el cap de velocidad nominal ningún costo se ve. Compararlos con el
  `GS 95 %` / ~52 fps de §8.18 es comparar dos regímenes, no dos configuraciones.
- **No dar por pendiente el test de los 12 bytes del `.WDD`.** Las tareas 6.2 y 6.3 de
  `ESTADO_ACTUAL.md` §N3 ya tienen resultado parcial (`.DB` firma `..FT` en 6-7; `.WDD`
  `byte1 = 0x02`, tamaños 16K/64K). Lo abierto es el **byte 0**, no el bloque entero.
- **No volver a auditar `Downloads/pipeline_metadev_ps2.txt`**: ya se hizo el 2026-08-27,
  y la auditoría está en `perfil-global/referencias/auditoria-pipeline-metadev.md`.
- **El emulador de la línea DLSS/texturas es `C:\Program Files\PCSX2\pcsx2-qt.exe`**, no el de
  los `lanzadores/*.bat` (ésos abren el fork PCSX2-MCP del reversing). Ya está en §8.17.

**NEXT ACTION, en orden de apalancamiento:**
1. **A/B visual pareado** — renombrar `replacements\`, mismo frame, dos capturas.
2. **Cuánto CUBRE el pack**: `DumpReplaceableTextures = true`, cargar `LEVEL_00`, contar. Hoy
   nadie sabe si cubre el 100 % del nivel o el 30 %, y eso decide si hay trabajo por delante.
3. **Costo en FPS con turbo**, método de R3 (`F6`/`F5`).
4. Regenerar mipmaps del pack (el gotcha de arriba).
5. Evaluar `PrecacheTextureReplacements` con 1,3 GB.

**MODEL/EFFORT REC:** Sonnet, esfuerzo medio, sin fan-out. El método ya está decidido (es el de
R0/R1/R3) y lo que queda es ejecutarlo y tabular. Sube a Opus sólo si un resultado contradice
lo anotado.
