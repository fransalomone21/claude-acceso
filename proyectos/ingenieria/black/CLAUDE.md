...
| `casos_dispatcher.py` | **de qué subsistema es cada tipo de módulo**, leyendo el SITIO DE LLAMADA de `FUN_0015ef48` (no el handler: ahí el dato no está). `mapa` / `familias` / `arrays` / `caso 0xNN` |
...
| `unit.py` | **el CONTENEDOR de geometría**: `Levels/Level_NN/Unit_NN.bin` con el layout leído AL CARGADOR (`FUN_0012eae8`). `niveles` / `header` / `modelos` / `modelo` / `autotest`. Su alarma está probada en rojo con `probar-unit.py` |
| `modelo.py` | **los VÉRTICES**: desarma el bloque de `submalla+0xC0` de un modelo — árbol BIH, caras de 4 índices y vértices `u16` sesgados. `submallas` / `vertices` / `verificar` / `obj` / `autotest`. Probada en rojo con `probar-modelo.py` |
