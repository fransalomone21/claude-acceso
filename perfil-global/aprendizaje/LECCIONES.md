...
### Read con pages falla en PDFs grandes: falta poppler/pdftoppm en Windows

**Regla:** en esta maquina, para extraer texto de un PDF grande, usar directamente python con pymupdf (ya instalado: import fitz o import pymupdf) para volcar el texto completo a un .txt con marcadores de pagina (f.write(f"=== PDF PAGE {i+1} ===\n") + page.get_text()), y despues Read/Grep sobre ese .txt en vez de reintentar Read con pages sobre el PDF. Si hace falta inspeccionar visualmente una pagina (verificar una formula, una figura), renderizarla aparte con page.get_pixmap(dpi=150).save(...) y leer el PNG resultante.

**Sintoma:** al llamar Read con parametro pages sobre un PDF de varios cientos de paginas, la herramienta tira un error mencionando que "pdftoppm is not installed", y no hay forma obvia de instalar poppler rapido en Windows dentro de la sesion.

**Costo:** dos subagentes en paralelo, en la misma sesion, perdieron tiempo repitiendo el mismo intento fallido con Read+pages antes de pivotar a pymupdf, porque no habia ningun registro previo que lo advirtiera  ·  general  ·  2026-08-24

### install.ps1 hace overwrite total de perfil-global, no instala solo lo pedido

**Regla:** install.ps1 no hace merge entre lo ya instalado y la rama nueva: es un overwrite total de CLAUDE.md + todas las carpetas con SKILL.md, y solo CLAUDE.md y settings.json quedan con backup .bak. Antes de correrlo desde una rama distinta de la ultima que instalo, comparar contra lo ya instalado (git diff --stat, o listar que skills trae esa rama) para no pisar contenido mas nuevo sin darse cuenta

**Sintoma:** se corrio install.ps1 solo para dejar disponible aprender.py/aprendizaje, pero tambien piso ~/.claude/CLAUDE.md (con backup) y, sin ningun backup, las skills cuadro-de-fase, engineering-orchestrator, lecciones-aprendidas, spec-interview y verify-before-build con las versiones mas viejas de la rama que se estaba instalando -- se perdio la linea Esfuerzo del cuadro de fase y unas 357 lineas de lecciones-aprendidas/SKILL.md

**Costo:** recien se noto al revisar el listado de skills disponibles despues de correr install.ps1 -- no hubo aviso en el momento  ·  general  ·  2026-08-24
...
Total: 41 lecciones, 0 exitos auditados.
