...
| `teoria-circuitos/` | Informes de laboratorio de Teoría de Circuitos (UNSAM, cátedra Sanca), en grupo de tres | **ACTIVO** — fases 1 a 4 **CERRADAS**; el Informe 1 entregado con **nota 7**. La corrección en papel del profe abrió la **fase 5**: destilar lo que corrigió a mano en reglas medibles. Salió `CRITERIO-CATEDRA.md` —las **15 marcas** traducidas a 21 reglas, con una hipótesis falsable sobre cómo lee— más `verificar-estilo.py`, que mide las 6 medibles sobre el **`.docx`** y no sobre el script que lo genera, y su saboteador en **9/9**. El **Informe 2 pasó de 9 problemas a 0** y está subido al Doc del grupo: **6 páginas**, medidas sobre el PDF que exporta **Google Docs** y no sobre el de LibreOffice, que con el mismo archivo da 7 y un renglón fantasma. De paso, el Drive: la raíz de apuntes es **pública por link** y ese permiso **se hereda**, así que la carpeta «PRIVADOS» que estaba adentro no era privada — las carpetas del grupo se mudaron **afuera**. **Fase 5 abierta: falta el Pre-Lab**, con los 4 errores técnicos de `CORRECCIONES.md` (el peor define −3 dB como caer al 30 % cuando es al 71 %). **Repo aparte**: la carátula lleva mails de compañeros |
