...
| [`metodo-agustin/`](proyectos/ingenieria/metodo-agustin/CLAUDE.md) | Pasar el método (pilares, reglas, skills, hooks, frenos, lecciones, cascada) a la notebook de Agustín **sin** los proyectos de Fran: un núcleo **generado** por script desde los dos repos, filtrado de lo personal, que viaja por GitHub. Sólo ida | **ACTIVO** — abrió el 2026-09-26 con la **fase 0 cerrada**: Windows, sólo ida, y el tope del plan de Claude entra como riesgo vigilado. Se descartó invitarlo a `perfil-global` tal cual («Fran» 103 veces, rutas fijas en 7 archivos, y el colaborador tendría escritura). **Fase 1 abierta: exportador + verificador de privacidad, con su saboteador en rojo 5 de 5.** De paso: `install.ps1` no tiene parámetro de destino, así que el del export **no se corre nunca acá** porque pisaría el perfil de Fran |
